...
public class Score {
    ...
    private static final int HI_X = SCREEN_WIDTH - ((SCORE_LENGTH * NUMBER_WIDTH + SCREEN_WIDTH / 100) * 2 + 
    		NUMBER_WIDTH * 2 + SCREEN_WIDTH / 100);
    ...
    public void draw(GraphicsContext gc) {
        ...
        for (int i = 0; i < SCORE_LENGTH; i++) {
            gc.drawImage(cropImage(numbers, scoreArray[SCORE_LENGTH - i - 1]), 
            		CURRENT_SCORE_X + i * NUMBER_WIDTH, SCORE_Y);
        }
        if (hiScore > 0) {
            int[] hiScoreArray = scoreToArray(hiScore);
            gc.setGlobalAlpha(0.5);
            for (int i = 0; i < SCORE_LENGTH; i++) {
                gc.drawImage(cropImage(numbers, hiScoreArray[SCORE_LENGTH - i - 1]), 
                		HI_SCORE_X + i * NUMBER_WIDTH, SCORE_Y);
            }
