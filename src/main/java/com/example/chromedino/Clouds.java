...
public class Clouds {
    ...
    public void isOutofScreen() {
        for (Iterator<Cloud> i = clouds.iterator(); i.hasNext(); ) {
            Cloud cloud = i.next();
            cloud.x -= speedX / 7;
            if (cloud.x + cloudwidthscaled < 0) i.remove();
        }
    }
    ...
    private void createClouds() {
        if (clouds.size() < CLOUDS_AMOUNT) {
            for (Cloud cloud : clouds) {
                if (cloud.x >= SCREEN_WIDTH - cloudwidthscaled) return;
            }
            if (Math.random() * 100 < CLOUD_PERCENTAGE) {
                clouds.add(new Cloud(new Image("cloud.png"), SCREEN_WIDTH, (int) (Math.random() * SCREEN_HEIGHT / 2)));
            }
        }
    }
    ...
    public void draw(GraphicsContext gc) {
        for (Cloud cloud : clouds) {
            gc.drawImage(cloud.cloudimage, cloud.x, cloud.y, cloud.cloudimage.getWidth(), cloud.cloudimage.getHeight());
        }
    }
}
